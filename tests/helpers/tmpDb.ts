...
/**
 * Creates an on-disk SQLite database in a unique temp directory for integration tests.
 * Returns the raw better-sqlite3 handle, the file path, and a cleanup function.
 *
 * NOTE: Does NOT import electron — the path is managed by the test itself.
 */
export function createTmpDb(): { sqlite: Database.Database; path: string; cleanup: () => void } {
  const dir = path.join(os.tmpdir(), `rh-test-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  // Apply full schema — copied verbatim from tests/helpers/db.ts (and ultimately from src/main/db/index.ts ensureSchema)
  // CRITICAL: Keep in sync with production ensureSchema()
  sqlite.exec(`
  ...
  // Apply incremental ALTER TABLE migrations (matches production ensureSchema)
  ...
  return { sqlite, path: dbPath, cleanup }
}
